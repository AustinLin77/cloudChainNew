...
export class ShopListPage {
  ...
  //初始化获得购物车数量
  ionViewWillEnter(){
    console.log(GlobalVariable.shopCount)
    this.total=0
  ...
  changeNum(){
    ...
    for(var i =0;i<this.data.length;i++){
      for(var b=0;b<this.data[i].content.length;b++){
        console.log(this.data[i].content[b].isChecked);
        if(this.data[i].content[b].isChecked){
          console.log("22222")
          this.total=Number((this.total+parseFloat(this.data[i].content[b].goodPrice)*this.data[i].content[b].buyNum).toFixed(2));
        }
      }
    }
