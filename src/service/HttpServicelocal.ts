...
import { APP_SERVE_URL_LOCAL } from "./Constants";

@Injectable()
export class HttpServicelocal {
  ...
  public getWithHeader(url: string, paramObj: any) {
     url = APP_SERVE_URL_LOCAL +url + this.toQueryString(paramObj);
  ...
  public getUser1(url: string, paramObj: any) {
    url =APP_SERVE_URL_LOCAL+url + this.toQueryString(paramObj);
    var authorization = 'Bearer ' + localStorage.getItem("token");
    var teamId = localStorage.getItem("teamId");
    var userName= localStorage.getItem("userName");

    let headers = new Headers({'Content-Type': 'application/json', 'Authorization': authorization, 'X-TenantId':teamId, "Prefer-Lang":"zh-CN","Accept": "application/json", "X-Logined-Sign": userName,"X-UserId":localStorage.getItem('userId'),});
    console.log(headers);
